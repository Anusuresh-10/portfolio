...
export default function Experience({ zones, activeZone, onSelect, orbitEnabled }) {
  return (<>
    ...
    {zones.map(zone=><Platform key={zone.id} zone={zone} isActive={activeZone===zone.id} onSelect={onSelect}/>)}
    <Avatar zones={zones} activeZone={activeZone}/>
    {orbitEnabled?<OrbitControls enablePan={false} minDistance={5} maxDistance={30}/>:<CameraRig zones={zones} activeZone={activeZone}/>}
  </>);
}
